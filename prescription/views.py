...
class PrescriptionDetailView(APIView):
    ...
    @swagger_auto_schema(
        ...
        operation_description="Retrieve detailed information about a specific prescription. Accessible by the doctor who created it or the patient it was created for.",
        manual_parameters=[
            openapi.Parameter(
                'prescription_id',
                openapi.IN_PATH,
        ...
        try:
            try:
                prescription = Prescription.objects.select_related(
                    'doctor', 'patient'
                ).prefetch_related('medications', 'lab_tests').get(id=prescription_id)
            ...
            if is_doctor:
                if prescription.doctor != user.doctor_profile:
                    return Response(
                        {"error": "You can only view prescriptions you created"},
                        status=status.HTTP_403_FORBIDDEN
                    )
            elif is_patient:
                if prescription.patient != user.patient_profile:
                    return Response(
