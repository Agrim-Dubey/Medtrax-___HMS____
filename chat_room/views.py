...
class PatientChatViewSet(viewsets.ViewSet):
    ...
    def list_groups(self, request):
        ...
        groups = ChatRoom.objects.filter(room_type='group', is_active=True).prefetch_related('participants')
...
class DoctorChatViewSet(viewsets.ViewSet):
    ...
    def accept_connection(self, request, pk):
        ...
        connection = get_object_or_404(
            DoctorConnection, pk=pk,
            to_doctor=request.user.doctor_profile, status='pending'
        )
    ...
    def reject_connection(self, request, pk):
        ...
        connection = get_object_or_404(
            DoctorConnection,
            pk=pk, to_doctor=request.user.doctor_profile, status='pending'
        )
...
class ChatRoomViewSet(viewsets.ViewSet):
    ...
    def send_message(self, request, pk):
        ...
        message = Message.objects.create(room=chat_room, sender=request.user, content=request.data.get('content', ''))
